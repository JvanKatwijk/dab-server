...
int32_t	r;
	...
	(void)rtlsdr_set_tuner_gain (device,
                              gains [theGain * gainsCount / 100]);
	if (autogain)
	   rtlsdr_set_agc_mode (device, 1);
	(void)(rtlsdr_set_center_freq (device, frequency));
	running	= true;
...
int	rtlsdrHandler::whoamI	(void) {
	return S_DABSTICK;
}

std::string rtlsdrHandler::deviceName (void) {
char	manufact [255];
char	product  [255];
char	serial   [255];
std::string	result;
