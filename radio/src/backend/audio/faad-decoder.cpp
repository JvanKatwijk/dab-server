...
#include	"faad-decoder.h"
	...
	faadDecoder::faadDecoder (audioOut_t soundOut, void *userData) {
...
bool    faadDecoder::initialize (stream_parms *p) {
long unsigned int sample_rate;
uint8_t channels;
 ...
 */
        ...
        int core_sr_index =
                     p -> dacRate ? (p -> sbrFlag ? 6 : 3) :
                               (p -> sbrFlag ? 8 : 5);   // 24/48/16/32 kHz
        int core_ch_config = get_aac_channel_configuration (p -> mpegSurround,
                                                            p -> aacChannelMode);
        if (core_ch_config == -1) {
...
void	faadDecoder::output (int16_t *buffer,
	                     int	size,
	                     bool	isStereo,
	                     int	rate) {
	if (soundOut == NULL)
